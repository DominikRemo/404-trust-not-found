...
const themeTitle = computed(() => ({ system: 'System theme', light: 'Light mode', dark: 'Dark mode' }[theme.value]))
...
<template>
  <div class="min-h-screen bg-page flex flex-col items-center px-6 py-12 gap-10 transition-colors duration-200">
    ...
    <button
      @click="cycleTheme"
      class="fixed top-4 right-4 z-50 w-10 h-10 rounded-xl bg-surface border border-border text-ink-dim shadow-sm hover:bg-raised transition-colors flex items-center justify-center cursor-pointer"
      :title="themeTitle"
    >
        ...
        <div class="flex flex-col sm:flex-row items-stretch">
          ...
          <div class="flex-1 px-8 py-7 flex flex-col gap-3">
            <div class="flex items-center gap-3">
              ...
              <h2 class="text-lg font-semibold text-ink-sub m-0">Host a Game</h2>
            </div>
          ...
          <div class="flex-1 px-8 py-7 flex flex-col gap-3">
            <div class="flex items-center gap-3">
              ...
              <h2 class="text-lg font-semibold text-ink-sub m-0">Join a Game</h2>
            </div>
            <p class="text-sm text-ink-dim leading-relaxed m-0">
    ...
    <div v-else class="flex flex-col gap-5 w-full max-w-lg">
      ...
      <div class="bg-surface border border-border rounded-2xl shadow-sm p-7 flex flex-col gap-5">
        ...
        <div v-if="isHost" class="flex flex-col gap-2">
          <span class="text-xs font-semibold uppercase tracking-wider text-ink-faint">Room Code</span>
          <div class="flex items-center gap-3">
          ...
          <p class="text-xs text-ink-faint m-0">Share this code with players who want to join.</p>
        </div>

        <!-- Editable display name -->
        <div class="flex flex-col gap-2">
          <span class="text-xs font-semibold uppercase tracking-wider text-ink-faint">Your Name</span>
    ...
    <Dialog v-model:visible="sessionEnded" modal header="Session Ended" :closable="false" class="w-96">
      <div class="flex flex-col items-center gap-4 py-4 text-center">
        <span class="text-5xl">🔌</span>
        <p class="text-ink-dim text-sm m-0">The host has closed this session.</p>
      </div>
      <template #footer>
        <Button label="Back to Lobby" icon="pi pi-home" fluid @click="reset" />
      </template>
